...
export class AnnouncementService extends MainService {
  ...
  async getAnnouncementDrafts() {
    return firstValueFrom(
      this.http
        .get(this.backend_url + '/announcement/drafts', {headers: this.headers})
        .pipe(map((d) => d))
    ).catch((e) => false);
  }
  ...
  async getAnnouncementPosted() {
    return firstValueFrom(
      this.http
        .get(this.backend_url + '/announcement/posted', {headers: this.headers})
        .pipe(map((d) => d))
    ).catch((e) => false);
  }
  ...
  async deleteAnnouncement(id:any) {
    return firstValueFrom(
      this.http
        .delete(this.backend_url + `/announcement/${id}`, {
