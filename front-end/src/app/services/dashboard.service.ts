...
export class DashboardService extends MainService {
  ...
  async current() {
    return firstValueFrom(this.http.get(this.backend_url + '/dashboard/program/score', {headers: this.headers}).pipe(map(d=>d))).catch((e) => false);
  }
  async details() {
    return firstValueFrom(this.http.get(this.backend_url + '/dashboard/program/details', {headers: this.headers}).pipe(map(d=>d))).catch((e) => false);
  }
  ...
  async getOrgProgRisk(phaseId: any) {
    return firstValueFrom(this.http.get(this.backend_url + `/dashboard/organizations/${phaseId}`, {headers: this.headers}).pipe(map(d=>d))).catch((e) => false);
  }
}