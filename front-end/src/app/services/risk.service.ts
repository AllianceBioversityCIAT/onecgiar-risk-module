...
export class RiskService extends MainService {
  ...
  getInitiativeCategories(id:number) {
    return firstValueFrom(
      this.http
        .get(this.backend_url + '/program/'+id+'/categories', { headers: this.headers })
        .pipe(map((d) => d))
    );
  }
  getInitiativesCategories() {
    return firstValueFrom(
      this.http
        .get(this.backend_url + '/program/all/categories', { headers: this.headers })
        .pipe(map((d) => d))
    );
  }
  getRiskUsers(id: number) {
    return firstValueFrom(
      this.http
        .get(this.backend_url + `/program/${id}/roles`, {
          headers: this.headers,
        })
        .pipe(map((d) => d))
    );
