...
export class MitigationStatusFormDialogComponent {
  constructor(
  ...
  mitigationForm = this.fb.group({
    ...
    title: this.fb.control(
      this.mitigationData?.element?.title || null,
      Validators.required
    ),
    description: this.fb.control(
      this.mitigationData?.element?.description || null,
      [Validators.required]
    ),
  });
  ...
  async onSubmit() {
    ...
    if (this.mitigationData.action == 'Edit mitigation') {
      if (this.mitigationForm.valid) {
        await this.mitigationStatusService.updateMitigationStatus(
          id,
      ...
      if (this.mitigationForm.valid) {
        await this.mitigationStatusService.addMitigationStatus(
          this.mitigationForm.value
        );
